...
class IntPC:
    ...
    def arb(self):
        self.rb += self.mem[self.registers[0]]
        self.pc += 2
    ...
    def run(self):
        while not self.halted:
            ...
            for i in range(op_argc):
                self.registers[i] = self.mem[self.pc + (i + 1)] if self.parameter_modes[i] == 0 else (self.rb + self.mem[self.pc + (i + 1)]) if self.parameter_modes[i] == 2 else (self.pc + (i + 1))
